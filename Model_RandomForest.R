...
#save model data for prediction tool
...
#plot the importance with mean decrease in purity
rf.fit <- rf.model.final%>%fit(formula = surv~.,
                               data = bake(data.recipe,new_data=train.data.select))%>%
  extract_fit_engine()##extract model for importance and SHAP
rf.importance <- vip(rf.fit,num_features = 13)##Figure 3
...
explanation.rf <- explain(
  model=fit(rf.model.final,formula = surv~.,
            data = bake(data.recipe,new_data=train.data.select)),
  x_explain=test.data.baked[34,-14],
...
rf.shap <- plot(explanation.rf, bar_plot_phi0 = FALSE,col = c("#e66f51","#8ab07d"),pch=20)##Figure 4
rf.importance$theme <- rf.shap$theme